...
func (a *ControllerApp) makeControllerUI(w fyne.Window, apps *Apps) fyne.CanvasObject {
	...
	a.Pins = container.NewBorder(container.NewGridWithColumns(2, widget.NewButtonWithIcon("Add a pin", theme.ContentAddIcon(), func() {
		...
		widget.NewButtonWithIcon("Copy code to clipboard", theme.ContentCopyIcon(), func() {
			if a.current != nil {
				log.Println(json.Marshal(BuildPythonCode(a.current)))
				bytes, _ := json.Marshal(BuildPythonCode(a.current))
				w.Clipboard().SetContent(string(bytes))
			}
		}),
		widget.NewLabel("pin"),
